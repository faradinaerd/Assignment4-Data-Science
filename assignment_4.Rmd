...
author: |
  | Faradina Eka Radwa
  | d.faradinaekaradwa@students.uu.nl
  | http://www.github.com/faradinaerd
date: April 2023
urlcolor: purple
linkcolor: purple
output: 
...
#Group with growth equal to 0: The mean growth is 2.46, and the median growth is 2.29. This suggests that, the group with growth equal to 0 has a moderate positive growth rate, as the mean is slightly higher than the median. Group with growth more than 0: The mean growth is 1.68, and the median growth is 1.92. This indicates that, the group with growth more than 0 has a slightly lower growth rate compared to the group with growth equal to 0, as both the mean and median values are lower.
...
# P-value test results in the previous analysis shows the number 0.06871 represents that there is 6% probability of the observed results, assuming the null hypothesis (no difference in means) is true. Since the p-value is greater than the significance level of 0.05, thus we failed to reject the null hypothesis.
...
# From the model and the data, we can tell that 'rgdp60' is included to show the existence of the correlation between (economic) growth and the gross domestic product in each country. The coefficient is the indication of the effect of 'rgdp60' on the 'growth' variable assuming all other variables are being held constant.
...
# According to the previous analysis, education is the main driver of economic growth. This can be evaluated by seeing the R-squared value, the value for education in R2 is significantly way bigger than others. 
...
```{r}
write.table(table, file = "table.doc", sep = ",", quote = FALSE, row.names = TRUE)
```
